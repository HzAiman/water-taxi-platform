...
void main() {
  group('OperatorRepository', () {
    test('setOnlineStatus mirrors operator presence document', () async {
      ...
      expect(
        presenceSnap.data()?[OperatorPresenceFields.isOnline],
        isTrue,
      );
    });
  });
  ...
  group('BookingRepository', () {
    test('rejectBooking uses operator presence to fully reject booking', () async {
      final firestore = FakeFirebaseFirestore();
      ...
      await firestore.collection(FirestoreCollections.bookings).doc('booking-1').set({
